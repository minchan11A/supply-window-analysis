...
body.push(H1('1. 개 요'));
body.push(note(
  '한 줄 요약 : 도서·격오지 부대의 보급 제약은 통념과 달리 풍속보다 시정(視程) 조건이 지배하며, ' +
  '현행 획일적 안전재고 기준을 공개 기상자료 기반의 부대별 산정 절차로 대체해야 한다. ' +
  '본 제안의 산출물은 특정 숫자가 아니라, 어느 부대에도 동일하게 적용 가능한 산정 절차이다.'
));
body.push(RP([
...
body.push(dataTable(
  ...
  [
    ...
    ['③', '위험 시기의 재발견', '전수단 동시불가 15일 중 7월이 5일로 최다 — 동절기가 아닌 하계에 집중'],
    ['④', '기준값의 일관성 확인', 'P95 방식과 재현기간 분석이라는 서로 다른 통계적 접근이 동일한 3일 수준을 도출'],
  ],
  { boldCols: [1] }
));
body.push(H2('◇ 제안 내용'));
body.push(B('부대별 기상 고립 대응 커버리지 산정 절차 도입 (백령도 적용 결과 : 3일)', { marker: '(1) ' }));
...
body.push(P('(1) 지배적 제약은 풍속이 아니라 시정 조건이다', { bold: true, color: NAVY, size: 20, before: 100, after: 55 }));
...
body.push(RP([
  ['① P95(연도별 최장 연속 고립일수) = 2일 + 보수적 여유분 1일 = '],
  ['3일', { bold: true }],
  [' / ② 재현기간 분석의 20년 재현수준 = '],
  ['3일', { bold: true }],
  [' (95% CI 2~5일) — 서로 다른 통계적 접근이 동일 수준을 도출'],
]));
body.push(RP([
  ['평균 최장 고립일수는 0.6일이지만, 이를 기준으로 삼으면 기상이 특히 나빴던 해(2016년 최장 2일, 연간 7일)에는 ' +
   '대응이 불가능하다. 커버리지의 목적은 평년 대응이 아니라 '],
  ['극단 상황 대응', { bold: true }],
  ['이므로 분포의 꼬리(tail)를 기준으로 삼아야 하며, 과산포지수 2.93이 이 논리를 뒷받침한다.'],
]));
body.push(note(
  '여유분 1일에 대하여 : 이는 통계적으로 도출된 값이 아니라 현 단계의 정책적 보수값이다. ' +
  '본 분석은 기상 조건만을 판정하므로 정비·인력·행정 사유에 의한 보급 무산을 포착하지 못하는 반면, ' +
  '기상이 막는데 보급이 성공하는 경우는 드물다. 즉 산출된 고립일수는 실제 위험의 하한(lower bound)이며, ' +
  '여유분 1일은 이 편향 방향을 보정하기 위한 잠정값이다. 향후 실제 보급 실패 데이터가 축적되면 ' +
  '해당 여유분을 실증적으로 보정한다.'
));
...
const sopRows = [
  ['STEP 1', '기상 확인', '향후 72시간 시정·풍속 예보 확인 (※ 72시간은 기본 확인 범위이며, 기상예보 정확도와 부대 보급주기에 따라 조정 가능)'],
  ['STEP 2', '수단별 가용성 판단', '선박 : 시정 1,000m↑ 풍속 14m/s↓ · 헬기 : 시정 1,600m↑ 풍속 15m/s↓ · 드론 : 풍속 8m/s↓ 시정 500m↑ → 1개 이상 운용 가능한 수단의 존재 여부 확인 (※ 해당 수단의 적재·운송능력 충족 여부는 기존 보급계획의 판단요소로 별도 검토)'],
  ['STEP 3', '재고 대비 확인', '현 재고 잔여일수 (      )일  vs  부대 안전재고 기준 (      )일  [= 커버리지 + 리드타임]'],
  ['STEP 4', '판 단', '재고 < 안전재고 기준 AND 금일 운용 가능 수단 존재 → 선제보급 실시(지휘관 결심) / 48시간 내 전 수단 제한 예상 → 선제보급 검토 / 그 외 → 정상'],
];
